...
public class NetClass {
    ...
    public String generateNetSource() {
        ...
        //Package
        this.netSource.append("package org.uam.aida.tscc.OP_models;").append(EOL);
        
        // Imports
        if (!Global.petriNet.getImportText().isEmpty()) {
            this.netSource.append(Global.petriNet.getImportText()).append(EOL);
        }
        //Common imports for every Net
        this.netSource.append("import org.uam.aida.tscc.business.*;").append(EOL);
        this.netSource.append("import org.uam.aida.tscc.APFE.TSWFnet.TSWFNet;").append(EOL);
        this.netSource.append("import org.uam.aida.tscc.APFE.timeseries_guards.*;").append(EOL);
        this.netSource.append("import org.uam.aida.tscc.APFE.time_series_log.LogEntry;").append(EOL);
        this.netSource.append("import com.moandjiezana.toml.Toml;").append(EOL);
        this.netSource.append("import static org.uam.aida.tscc.APFE.timeseries_guards.LogicalTSG.LogicalExpression;").append(EOL);
        this.netSource.append("import java.util.logging.Level;").append(EOL);
        this.netSource.append("import java.util.logging.Logger;").append(EOL);
        this.netSource.append("import com.google.common.collect.Range;").append(EOL);
        this.netSource.append(EOL);

        if (!Global.petriNet.getImplementText().isEmpty()) {
            this.netSource.append("public class ").append(Global.petriNet.getLabel()).append(" extends TSWFNet implements ").append(Global.petriNet.getImplementText()).append(" {").append(EOL).append(EOL);
        } else {
            //this.netSource.append("public class ").append(Global.petriNet.getLabel()).append(" extends PetriNet {").append(EOL);
            this.netSource.append("public class ").append(Global.petriNet.getLabel()).append(" extends TSWFNet {").append(EOL);
        }
        
        //Logger
        this.netSource.append(EOL)
                .append("  private static final Logger LOG = Logger.getLogger(")
                .append(Global.petriNet.getLabel())
        ...
        if (!places.isEmpty()) {
            ...
            for (int i = 0; i < places.size(); i++) {
                ...
                this.netSource.append("  private Place ")
                        .append(place.getId())
                        //.append("_")
                        //.append(place.getLabel())
                        .append(" = new Place(\"" + place.getId() + "\");")
                        .append(EOL);
            }
        }
        ...
        if (!transitions.isEmpty()) {
            ...
            for (int i = 0; i < transitions.size(); i++) {
                ...
                this.netSource.append("  private Transition ")
                        .append(transition.getId())
                        //.append("_")
                        //.append(transition.getLabel())
                        .append(" = new Transition(\"" + 
                                transition.getId() + 
                                "\",\"" + 
                                addSlashes(transition.getGuardText()) + 
                                "\",\"" + 
                                addSlashes(transition.getExecuteText()) + 
                                "\") {")
                        .append(EOL);
                ...
                //this.netSource.append("  };").append(EOL); CHANGE
                
                //CHANGE - Transition Execute text
                this.netSource.append("         public TokenSet execute() {").append(EOL);
                this.netSource.append("                 return new TokenSet(" + transition.getExecuteText() + ");").append(EOL);
                this.netSource.append("         }").append(EOL);
        ...
        if (!inputArcs.isEmpty()) {
            ...
            for (int i = 0; i < inputArcs.size(); i++) {
                ...
                this.netSource.append("  private InputArc ")
                        .append(inputArc.getId())
                        .append(" = new InputArc(\"" + inputArc.getId() + "\",")
                        .append(inputArc.getPlace().getId())
                        .append(",")
                        .append(inputArc.getTransition().getId())
                        .append(",")
                        .append(" \"" + addSlashes(inputArc.getEvaluateText()) + "\" ")
                        .append(",")
                        .append(" \"" + addSlashes(inputArc.getExecuteText()) + "\" ")
                        .append(") {")
                        .append(EOL);
                this.netSource.append("         public boolean evaluate() {").append(EOL);
                this.netSource.append("                 return " + addSlashes(inputArc.getEvaluateText()) + ";").append(EOL);
                //this.netSource.append("                  " + inputArc.getEvaluateText()).append(EOL);
                this.netSource.append("         }").append(EOL);
                this.netSource.append("         public TokenSet execute() {").append(EOL);
        ...
        if (!OutputArcs.isEmpty()) {
            ...
            for (int i = 0; i < OutputArcs.size(); i++) {
                ...
                if (expression.countTokens() == 2) {
                    ...
                    //TODO Esto se ha cambiado. No se añade retraso a los tokens
        ...
        for (int i = 0; i < transitions.size(); i++) {
            ...
            this.netSource.append("     " + transition.getId() + ".setTimeScope(Range.closed(" + 
                    transition.getTimeScope().lowerEndpoint() + "L" + 
                    "," + 
                    transition.getTimeScope().upperEndpoint() + "L" + 
                    "));").append(EOL);
            this.netSource.append("     " + transition.getId() + ".setGuardCondition(" + transition.getGuardConditionText() + ");").append(EOL);
        ...
        // Add trivial configure method
        this.netSource.append(EOL).append("public void configure(Toml cfg) {super.configure(cfg);}").append(EOL);
